...
def generate_mcqs(pdf_content, difficulty_level, num_questions=10, db_folder="chroma_db", temp_file_path=None):
    ...
    # Create a Chroma database with a specific directory for the current PDF
    persist_directory = os.path.join(db_folder, generate_pdf_id(temp_file_path))

    # Ensure directory exists only if needed
    if not os.path.exists(persist_directory):
        os.makedirs(persist_directory)

    db = Chroma.from_documents(documents=pdf_content, embedding=embeddings, persist_directory=persist_directory)
